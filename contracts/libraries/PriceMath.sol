...
library PriceMath {
    ...
    function getSqrtRatioAtPrice(address token0, address token1, uint256 price) public pure returns (uint160 sqrtPriceX96) {
        uint256 base = 1e18;
        if (token0 > token1) {
    ...
    function getPriceAtSqrtRatio(address token0, address token1, uint160 sqrtPriceX96) internal pure returns (uint256 price) {
